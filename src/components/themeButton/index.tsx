        ...
        <svg width="32" height="32" fill="none">
          ...
          <defs>
            <linearGradient id="paint0_linear" x1="2" y1="2" x2="30" y2="30" gradientUnits="userSpaceOnUse">
              <stop className="transition-all duration-200" stopColor={theme === 'dark' ? "#d4d4d8" : "#FACC15"}></stop><stop className="transition-all duration-200" offset="1" stopColor={theme === 'dark' ? "#d4d4d8" : "#FA9D16"}></stop>
            </linearGradient>
          </defs>
        </svg>
        <button onClick={onClickModernToggle} role="switch" tabIndex={0} className={`inline-flex items-center px-0.5 rounded-full h-9 transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-offset-white focus-visible:ring-gray-500 focus:outline-none w-16 bg-green-400 dark:bg-mtd-300 ${theme === 'dark' && ''}`} aria-checked={theme === 'dark' ? true : false}  >
          <span className="sr-only">Enable dark mode</span>
          <span className={`shadow-lg bg-white rounded-full w-8 h-8 transition-transform duration-500`} style={{ transform: theme === 'dark' ? 'translateX(1.8rem)' : '' }}></span>
        </button>
        <svg width="24" height="24" fill="currentColor" className="transition-colors duration-200 text-gray-800 dark:text-mtd-300">
